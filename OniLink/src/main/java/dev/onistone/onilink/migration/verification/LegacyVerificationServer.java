...
import dev.onistone.onilink.config.LegacyVerificationConfig;
...
public final class LegacyVerificationServer implements AutoCloseable {
    private final LegacyVerificationConfig config;
    private final PendingJoinRegistry pendingJoins;
    private final VerificationSigner signer;
    private HttpServer server;
    private ExecutorService executor;

    public LegacyVerificationServer(LegacyVerificationConfig config, PendingJoinRegistry pendingJoins, Clock clock) {
        if (config == null) {
    ...
    public void start() throws IOException {
        ...
        if (LegacyVerificationConfig.DEFAULT_SHARED_SECRET.equals(config.sharedSecret())) {
            System.out.println("WARNING: legacyVerification.sharedSecret is still the default development value.");
        }
    }
    ...
    private void handleVerify(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            if (!signer.verify(request, require(query, "signature"))) {
                System.out.printf(
                        "WARNING: Deprecated migration verifier request for %s from %s had a bad signature; this path must remain disabled in production.%n",
                        request.name(),
                        exchange.getRemoteAddress()
                );
