...
public final class AvailableCommandsInjector {
    ...
    public AvailableCommandsPacket inject(AvailableCommandsPacket packet) {
        ...
        if (registry.commands().stream().anyMatch(command -> visible.test(command.name()))
                && existing.add(ROOT_COMMAND)) {
            injected.add(toCommandData(freeTextType));
        }
    ...
    private CommandData toCommandData(CommandParam freeTextType) {
        return new CommandData(
                ROOT_COMMAND,
                "OniLink proxy administration and backend routing",
                ...
                null,
                List.of(),
                overloadsFor(freeTextType)
        );
    }

    private CommandOverloadData[] overloadsFor(CommandParam freeTextType) {
        List<CommandOverloadData> overloads = new ArrayList<>();
        for (ProxyCommand command : registry.commands()) {
            if (!visible.test(command.name())) continue;
            CommandParamData action = fixedEnumParameter(
                    "action", "OniLinkAction_" + command.name(), List.of(command.name()), false);
            switch (command.name()) {
                case "server" -> {
                    overloads.add(new CommandOverloadData(false, new CommandParamData[]{action}));
                    overloads.add(new CommandOverloadData(false, new CommandParamData[]{
                            action, backendNameParameter("name")}));
                }
                case "send" -> overloads.add(new CommandOverloadData(false, new CommandParamData[]{
                        action, playerParameter("player", false), backendNameParameter("server")}));
                case "perm" -> overloads.add(new CommandOverloadData(false, new CommandParamData[]{
                        action,
                        fixedEnumParameter("permission_action", "OniLinkPermissionActions", PERM_ACTIONS, false),
                        playerParameter("player", true),
                        fixedEnumParameter("node", "OniLinkPermissionNodes", permissionNodes(), true)
                }));
                case "alert" -> overloads.add(freeTextType == null
                        ? new CommandOverloadData(false, new CommandParamData[]{action})
                        : new CommandOverloadData(false, new CommandParamData[]{
                                action, freeTextParameter("message", freeTextType)}));
                default -> overloads.add(new CommandOverloadData(false, new CommandParamData[]{action}));
            }
        }
        return overloads.toArray(CommandOverloadData[]::new);
    }
