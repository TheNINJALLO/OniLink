...
public record ProxyConfig(
        ...
        LegacyVerificationConfig legacyVerification,
        ProxyPolicy policy,
        String motd,
        String subMotd,
        String gameType,
    ...
    public ProxyConfig {
        ...
        java.util.HashSet<String> forwardingSecretSources = new java.util.HashSet<>();
        for (BackendConfig configuredBackend : backends.values()) {
            BackendForwardingConfig forwarding = configuredBackend.forwarding();
            if (!forwarding.enabled()) continue;
            String source = !forwarding.activeSecretEnv().isEmpty()
                    ? "env:" + forwarding.activeSecretEnv()
                    : "file:" + Path.of(forwarding.activeSecretFile()).toAbsolutePath().normalize();
            if (!forwardingSecretSources.add(source)) {
                throw new IllegalArgumentException("Every enabled backend requires a unique OniForward secret source");
            }
        }
        if (hubBackendName == null || hubBackendName.isBlank()) {
            throw new IllegalArgumentException("hubBackendName cannot be blank");
        }
        if (legacyVerification == null) {
            throw new IllegalArgumentException("legacyVerification cannot be null");
        }
        if (policy == null) {
     ...
     */
    private static final String DEFAULT_CONFIG_RESOURCE = "/onilink.example.properties";
    ...
    private static void writeDefaultConfig(Path path) throws IOException {
        ...
        try (OutputStream output = Files.newOutputStream(path)) {
            defaultProperties().store(output, "OniLink Bedrock proxy configuration");
        }
    }
    ...
    public static ProxyConfig from(Properties properties, Path configDir) {
        ...
        BackendConfig defaultBackend = new BackendConfig(
                ...
                forwardingFor(properties, backendName, configDir)
        );
        Map<String, BackendConfig> backends = backendConfigs(properties, defaultBackend, configDir);
        ...
        // Retained only so old data/configuration can be diagnosed during migration. The blocking
        // callback protocol can never be enabled in the production authentication path.
        if (booleanProperty(properties, "legacyVerification.enabled", false)) {
            throw new IllegalArgumentException(
                    "legacyVerification cannot be enabled; configure OniForward and OniBridge");
        }
        boolean legacyVerificationEnabled = false;
        String legacyVerificationHost = properties.getProperty("legacyVerification.host", "127.0.0.1");
        int legacyVerificationPort = intProperty(properties, "legacyVerification.port", 19135);
        ...
        return new ProxyConfig(
                ...
                hubBackendName,
                backendProtocol,
                new LegacyVerificationConfig(
                        legacyVerificationEnabled,
                        new InetSocketAddress(legacyVerificationHost, legacyVerificationPort),
                        properties.getProperty(
                                "legacyVerification.sharedSecret",
                                LegacyVerificationConfig.DEFAULT_SHARED_SECRET
                        ),
                        intProperty(properties, "legacyVerification.pendingJoinTtlMillis", 15_000),
                        intProperty(properties, "legacyVerification.requestSkewMillis", 30_000)
                ),
                new ProxyPolicy(
                        failover,
                ...
                properties.getProperty("motd", "OniLink"),
    ...
    public static Properties defaultProperties() {
        ...
        properties.setProperty("forwarding.proxyId", "edge-1");
        properties.setProperty("backend.default.forwarding.enabled", "true");
        properties.setProperty("backend.default.forwarding.bridgeId", "default-main");
        properties.setProperty("backend.default.forwarding.activeKeyId", "key-1");
        properties.setProperty("backend.default.forwarding.activeSecretEnv", "ONIBRIDGE_FORWARDING_SECRET");
        properties.setProperty("backend.default.forwarding.tokenLifetimeMillis", "5000");
        ...
        properties.setProperty("motd", "OniLink");
    ...
    private static Map<String, BackendConfig> backendConfigs(
        ...
        for (String name : Arrays.stream(backendNames.split(","))
            ...
            backends.put(normalizedName, new BackendConfig(
                    ...
                    forwardingFor(properties, name, configDir)
            ));
        }
        return Collections.unmodifiableMap(backends);
    }

    private static BackendForwardingConfig forwardingFor(Properties properties, String backendName, Path configDir) {
        String prefix = "backend." + backendName + ".forwarding.";
        boolean enabled = booleanProperty(properties, prefix + "enabled", true);
        String secretFile = ConfigValues.stripInlineComment(properties.getProperty(prefix + "activeSecretFile", "")).trim();
        String previousFile = ConfigValues.stripInlineComment(properties.getProperty(prefix + "previousSecretFile", "")).trim();
        String activeSecretEnv = properties.getProperty(
                prefix + "activeSecretEnv",
                secretFile.isEmpty()
                        ? "ONIBRIDGE_FORWARDING_SECRET" + ("default".equalsIgnoreCase(backendName)
                                ? ""
                                : "_" + backendName.replaceAll("[^A-Za-z0-9]", "_").toUpperCase(Locale.ROOT))
                        : ""
        );
        if (configDir != null) {
            if (!secretFile.isEmpty()) secretFile = configDir.resolve(secretFile).normalize().toString();
            if (!previousFile.isEmpty()) previousFile = configDir.resolve(previousFile).normalize().toString();
        }
        return new BackendForwardingConfig(
                enabled,
                properties.getProperty("forwarding.proxyId", "edge-1"),
                properties.getProperty(prefix + "bridgeId", backendName + "-main"),
                properties.getProperty(prefix + "activeKeyId", "key-1"),
                activeSecretEnv,
                secretFile,
                properties.getProperty(prefix + "previousKeyId", ""),
                properties.getProperty(prefix + "previousSecretEnv", ""),
                previousFile,
                intProperty(properties, prefix + "tokenLifetimeMillis", 5_000)
        );
    }
