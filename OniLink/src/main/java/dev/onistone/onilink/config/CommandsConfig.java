 ...
 *
 * <p>The primary proxy surface is {@code /onilink <subcommand>}. Bare names and unknown
 * subcommands remain backend-authoritative. The qualifier field is retained only as an inert
 * migration setting, so an existing properties file cannot silently enable top-level aliases.</p>
 *
