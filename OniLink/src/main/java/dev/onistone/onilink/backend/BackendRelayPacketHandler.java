...
import dev.onistone.onilink.command.AvailableCommandsInjector;
import dev.onistone.onilink.codec.CodecDefinitionState;
import dev.onistone.onilink.config.BackendConfig;
import dev.onistone.onilink.diagnostics.PacketViolation;
import dev.onistone.onilink.diagnostics.ProtocolFault;
...
import dev.onistone.onilink.migration.verification.PendingJoin;
...
public final class BackendRelayPacketHandler implements BedrockPacketHandler {
    ...
    private void syncDefinitionState(BedrockPacket packet) {
        if (packet instanceof StartGamePacket startGame) {
            ...
            warnIfLegacyVerificationMissing();
     ...
     * {@link dev.onistone.onilink.registry.ItemPaletteMapping}. On a later backend these packets are
    ...
    private void warnIfLegacyVerificationMissing() {
        if (pendingJoin == null) {
            return;
        }
        ...
        System.out.printf(
                "WARNING: Backend %s sent StartGame for %s through the deprecated verifier path; production authentication requires OniForward and OniBridge.%n",
                backendName,
                pendingJoin.name()
        );
    }
