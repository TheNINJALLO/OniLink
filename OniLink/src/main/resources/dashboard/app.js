...
function healthClass(status) {
  return ["online", "offline", "degraded"].includes(status) ? status : "checking";
}

function applyRole() {
  $$(".operator-only").forEach(element => element.classList.toggle("role-hidden", !hasRole("operator")));
  $$(".admin-only").forEach(element => element.classList.toggle("role-hidden", !hasRole("admin")));
  $$(".owner-only").forEach(element => element.classList.toggle("role-hidden", !hasRole("owner")));
  $("#identity-name").textContent = model.principal?.username || "Not signed in";
  $("#identity-role").textContent = model.principal?.role || "Offline";
  $("#avatar").textContent = (model.principal?.username || "?").slice(0, 1).toUpperCase();
...
async function loadRuntime() {
  const [state, players, backends] = await Promise.all([
    api("/api/state"), api("/api/players"), api("/api/backends")
  ]);
  model.state = state;
...
function renderState() {
  ...
  const healthy = model.backends.filter(item => item.health?.status === "online").length;
  $("#metric-healthy").textContent = `${healthy} reporting online`;
  $("#metric-uptime").textContent = formatDuration(state.uptimeMillis);
  $("#metric-started").textContent = `Started ${new Date(state.startedAt).toLocaleString()}`;
  $("#metric-memory").textContent = formatBytes(state.memoryUsedBytes);
  $("#metric-memory-max").textContent = `${formatBytes(state.memoryMaxBytes)} maximum`;
  $("#refresh-time").textContent = `Updated ${new Date(state.timestamp).toLocaleTimeString()}`;
  $("#runtime-facts").innerHTML = `
    ...
    <div><dt>CPU load</dt><dd>${Number(state.systemLoadAverage) < 0 ? "Unavailable" : Number(state.systemLoadAverage).toFixed(2)}</dd></div>`;
...
function renderOverviewBackends() {
  ...
  target.innerHTML = model.backends.slice(0, 6).map(backend => {
    const status = backend.health?.status || "checking";
    const latency = backend.health?.latencyMillis >= 0 ? `${backend.health.latencyMillis} ms` : status;
    return `<div class="health-row"><i class="${healthClass(status)}"></i><b>${escapeHtml(backend.name)}</b><small>${escapeHtml(latency)}</small></div>`;
  }).join("");
}
...
function renderPlayers() {
  ...
  body.innerHTML = model.players.map(player => {
    const actions = hasRole("operator") ? `<div class="player-actions">
      ...
      <button class="mini-button" data-player="${escapeHtml(player.name)}" data-action="disconnect">Disconnect</button></div>` : "";
    return `<tr><td><b>${escapeHtml(player.name)}</b><small>${escapeHtml(player.address)}</small></td><td>${escapeHtml(player.backend)}</td><td>${escapeHtml(player.protocol)}</td><td>${formatDuration(player.connectedMillis)}</td><td><span class="badge ${player.switching ? "warn" : ""}">${player.switching ? "Switching" : player.joinedWorld ? "Playing" : "Joining"}</span></td><td class="operator-only">${actions}</td></tr>`;
  }).join("");
  applyRole();
}
...
function renderBackends() {
  ...
  grid.innerHTML = model.backends.map(backend => {
    const health = backend.health || {status: "checking", latencyMillis: -1};
    const flags = [backend.default ? "Default" : "", backend.hub ? "Hub" : "", backend.forwarding ? "Forwarding" : ""].filter(Boolean);
    return `<article class="panel backend-card"><div class="backend-top"><div><p class="eyebrow">${escapeHtml(backend.protocol)}</p><h3>${escapeHtml(backend.name)}</h3><span class="endpoint">${escapeHtml(backend.host)}${backend.port ? `:${escapeHtml(backend.port)}` : ""}</span></div><span class="badge"><i class="health-dot ${healthClass(health.status)}"></i>${escapeHtml(health.status)}</span></div><div class="backend-flags">${flags.map(flag => `<span class="badge">${escapeHtml(flag)}</span>`).join("") || `<span class="badge">Route</span>`}</div><div class="backend-stats"><div><span>Players</span><b>${escapeHtml(backend.players)}</b></div><div><span>Latency</span><b>${health.latencyMillis >= 0 ? `${health.latencyMillis} ms` : "—"}</b></div><div><span>Probe</span><b>${health.checkedAt ? new Date(health.checkedAt).toLocaleTimeString() : "Pending"}</b></div></div></article>`;
  }).join("");
  renderOverviewBackends();
}
...
async function loadAudit() {
  ...
  $("#audit-output").textContent = result.lines.join("\n") || "Audit log is empty.";
}
...
async function loadUsers() {
  ...
  $("#user-list").innerHTML = result.users.map(user => `<div class="user-row"><div><b>${escapeHtml(user.username)}</b><small>${user.totpEnabled ? "TOTP enabled" : "Password only"}</small></div><span class="badge">${escapeHtml(user.role)}</span>${user.role === "owner" ? "" : `<button class="button danger-outline" data-delete-user="${escapeHtml(user.username)}">Remove</button>`}</div>`).join("");
}
...
async function navigate(page) {
  ...
  try {
    ...
    if (["overview", "players", "backends"].includes(page)) await refreshRuntime();
  } catch (error) { notice(error.message, true); }
}

function openPlayerAction(action, player) {
  model.action = action;
  const labels = {transfer: "Transfer player", disconnect: "Disconnect player", trace: "Trace packets"};
  ...
  $("#action-form [name=backend]").innerHTML = model.backends.map(backend => `<option value="${escapeHtml(backend.name)}">${escapeHtml(backend.name)}</option>`).join("");
...
$("#login-form").addEventListener("submit", async event => {
  ...
  try { await completeAuthentication(await api("/api/login", {method: "POST", body: data})); }
  catch (error) { $("#auth-error").textContent = error.message; $("#auth-error").hidden = false; }
});
$("#setup-form").addEventListener("submit", async event => {
  event.preventDefault();
  const data = Object.fromEntries(new FormData(event.currentTarget));
  try { await completeAuthentication(await api("/api/setup", {method: "POST", body: data})); }
  catch (error) { $("#auth-error").textContent = error.message; $("#auth-error").hidden = false; }
});
...
$("#action-form").addEventListener("submit", async event => {
  ...
  const data = Object.fromEntries(new FormData(event.currentTarget));
  try {
    const result = await api(`/api/action/${model.action}`, {method: "POST", body: data});
    $("#action-dialog").close(); notice(result.message); await refreshRuntime();
  } catch (error) { notice(error.message, true); }
});
...
$("#refresh-audit").addEventListener("click", () => loadAudit().catch(error => notice(error.message, true)));
$("#download-support").addEventListener("click", async () => {
  try {
    const response = await fetch("/api/support-bundle", {headers: {Authorization: `Bearer ${model.token}`}});
    if (!response.ok) throw new Error((await response.json()).error || "Support download failed");
    const link = document.createElement("a"); link.href = URL.createObjectURL(await response.blob());
    link.download = `onilink-support-${Date.now()}.zip`; link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 2000);
  } catch (error) { notice(error.message, true); }
});
$("#shutdown-proxy").addEventListener("click", async () => {
  if (!confirm("Gracefully stop OniLink now? Connected players will be disconnected.")) return;
  try { const result = await api("/api/shutdown", {method: "POST"}); notice(result.message); }
  catch (error) { notice(error.message, true); }
});
...
$("#user-list").addEventListener("click", async event => {
  ...
  if (!button || !confirm(`Remove dashboard account ${button.dataset.deleteUser}?`)) return;
  try { await api("/api/users", {method: "DELETE", body: {username: button.dataset.deleteUser}}); await loadUsers(); notice("Dashboard account removed."); }
  catch (error) { notice(error.message, true); }
});
$("#password-form").addEventListener("submit", async event => {
  event.preventDefault();
  try { await api("/api/account/password", {method: "POST", body: Object.fromEntries(new FormData(event.currentTarget))}); notice("Password changed. Sign in again."); signOut(false); }
  catch (error) { notice(error.message, true); }
});
$("#begin-totp").addEventListener("click", async () => {
  try { const result = await api("/api/account/totp/begin", {method: "POST"}); model.totpSecret = result.secret; $("#totp-secret").textContent = result.secret; $("#totp-uri").textContent = result.uri; $("#totp-setup").hidden = false; }
  catch (error) { notice(error.message, true); }
});
$("#enable-totp").addEventListener("click", async () => {
  try { await api("/api/account/totp/enable", {method: "POST", body: {secret: model.totpSecret, code: $("#totp-code").value}}); notice("TOTP enabled. Sign in again."); signOut(false); }
  catch (error) { notice(error.message, true); }
});
$("#disable-totp").addEventListener("click", async () => {
  const password = prompt("Enter your current password:"); if (password === null) return;
  const code = prompt("Enter the six-digit authenticator code:"); if (code === null) return;
  try { await api("/api/account/totp/disable", {method: "POST", body: {password, code}}); notice("TOTP disabled. Sign in again."); signOut(false); }
  catch (error) { notice(error.message, true); }
});
