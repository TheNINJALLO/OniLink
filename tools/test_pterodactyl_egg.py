...
class PterodactylEggTests(unittest.TestCase):
    ...
    def test_installer_verifies_bootstrap_before_install_and_preserves_config(self) -> None:
        installation = self.egg["scripts"]["installation"]
        script = installation["script"]
        self.assertEqual("ghcr.io/ptero-eggs/installers:debian", installation["container"])
...
""", encoding="utf-8")
            ...
            completed = subprocess.run(
                ["bash", str(UPDATER_PATH)], cwd=root, env=environment,
...
""", encoding="utf-8")
    ...
    def test_variables_are_unique_and_secrets_are_admin_only(self) -> None:
        ...
        self.assertEqual("v0.1.0-candidate.3", variables["ONILINK_VERSION"]["default_value"])
