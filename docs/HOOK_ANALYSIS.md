...
The Windows synthetic executable hook harness passed under MSVC, including identity substitution, direct rejection, prior-call chaining, expected-byte checks, double-install rejection, rollback, and identity commit. A real Windows BDS 1.26.44.3/Endstone 0.11.9 offline-mode lifecycle then loaded and enabled the DLL, installed the exact hook, disabled it, and exited cleanly. The profile-specific Linux build and the same synthetic hook harness pass on GitHub's Ubuntu 22.04 runner. Neither platform has the complete recorded live client join/storage matrix or human profile approval. Both profiles therefore remain candidates and production use fails closed by default.
