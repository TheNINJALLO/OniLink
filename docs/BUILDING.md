...
The release `.so` is built natively by the Ubuntu 22.04 GitHub workflow with LLVM 18, libc++ 18, and libc++abi 18 packages built for Jammy. The workflow runs CTest and `tools/check_linux_abi.py` before packaging. The ABI check rejects imports newer than `GLIBC_2.35`, a `libstdc++.so.6` dependency, and unresolved `std::__cxx11` symbols; it also requires direct or host-resolved libc++ ABI evidence. This proves a native Linux build and synthetic harness execution; it does not replace live BDS/Endstone acceptance or human profile review.
