...
| Java proxy transport, authentication, routing, failover, switching, translation, resources, registries, permissions, addons and security controls | implemented but awaiting live validation | The Java 21 build passes 397 tests and produces `OniLink.jar`. No live network/runtime claim follows from unit tests. |
| `/onilink` merge/interception and backend command transparency | implemented but awaiting live validation | Focused Java tests pass for one root, exact bare-command forwarding and collision behavior; no live BDS command fixture ran. |
| Strict TOML, secret loading, plugin commands, diagnostics and migration service | implemented but awaiting live validation | Exact-profile Windows and Linux Endstone plugin candidates compile. `/onibridge` uses the public command API and outgoing command packets are observed but never modified. |
| Pre-storage native XUID hook on Linux | implemented but awaiting live validation | Exact ELF hash/ABI/call/helper/signature evidence and generated adapter exist; `onibridge.so` cross-compiles. The harness has not executed on Linux and no live BDS join ran. |
...
| Minimal real-BDS ABI headers and profiles | implemented and integration-tested | Independent generated headers/profiles validate against the exact cached Linux and Windows executables. They remain candidates pending human/live gates. |
| Native Linux/Windows artifacts and release package | implemented but awaiting live validation | Both candidate binaries and a candidate-aware manifest/profile bundle are produced without BDS-owned files. The manifest reports `production_ready=false`. |
...
| Live identity persistence, policies, real IP and command acceptance | unsupported with exact technical reason | The BDS files are available for analysis, but no Xbox/OniLink client acceptance session or Linux runtime was available; no live Linux or Windows server test was completed. |
