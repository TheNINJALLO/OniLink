...
| Linux candidate | ELF shared object cross-compiled | build only; no Linux runtime test |
...
No live client join has been completed on either platform. Windows plugin load/hook lifecycle is proven, but required acceptance still includes valid/direct/unsigned/tampered/expired/replayed/untrusted joins; two-player concurrency; restart/rejoin inventory and Ender Chest; all remaining storage fields; bans/allowlist/operators/permissions; real IP; Python and C++ Endstone command fixtures; aliases/overloads/enums/soft enums/tab completion/output; switching; and protocol translation. Linux additionally needs plugin load/hook lifecycle and shutdown cleanup. OniBridge-Geyser additionally needs a live Geyser 2.11/Floodgate join, direct-listener rejection, real-IP observation on Java, reconnect, and backend-switch testing.
