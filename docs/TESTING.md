...
| OniBridge Linux CTest | 2 passed on GitHub Ubuntu 24.04 | native unit + synthetic executable hook harness |
| Linux candidate package | Native ELF64 x86-64 build and checksum-verified release bundle | build/package evidence; no live BDS client join |
...
No live client join has been completed on either platform. Windows plugin load/hook lifecycle is proven, but required acceptance still includes valid/direct/unsigned/tampered/expired/replayed/untrusted joins; two-player concurrency; restart/rejoin inventory and Ender Chest; all remaining storage fields; bans/allowlist/operators/permissions; real IP; Python and C++ Endstone command fixtures; aliases/overloads/enums/soft enums/tab completion/output; switching; and protocol translation. Linux has native build and synthetic hook-harness evidence but additionally needs BDS plugin load/hook lifecycle, shutdown cleanup, and the same live join/storage matrix. OniBridge-Geyser additionally needs a live Geyser 2.11/Floodgate join, direct-listener rejection, real-IP observation on Java, reconnect, and backend-switch testing.
