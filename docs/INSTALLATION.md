...
> [!IMPORTANT]
> `v0.1.4` is the production release for the exact Linux BDS `1.26.44.3` + Endstone `0.11.9` profile. Its manifest reports `production_ready=true`; keep `allow_unreviewed_profile=false` so unknown or unapproved profiles remain blocked.
...
Download every required file from the [v0.1.4 release](https://github.com/TheNINJALLO/OniLink/releases/tag/v0.1.4), including `SHA256SUMS`. On Linux:
...
```bash
cp /path/to/onibridge-0.1.4-bds-1.26.44.3-linux-x86_64.so plugins/
chmod 0644 plugins/onibridge-0.1.4-bds-1.26.44.3-linux-x86_64.so
```
