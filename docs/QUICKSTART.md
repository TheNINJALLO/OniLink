...
This guide brings up the production OniLink `v0.1.2` release in front of one BDS `1.26.44.3` + Endstone `0.11.9` backend. The included exact Linux profile is production-approved and still fails closed on every mismatched runtime detail.
...
- The [current release](https://github.com/TheNINJALLO/OniLink/releases/tag/v0.1.2).
...
1. Stop BDS.
2. Copy `onibridge-0.1.2-bds-1.26.44.3-linux-x86_64.so` into the Endstone `plugins/` directory.
