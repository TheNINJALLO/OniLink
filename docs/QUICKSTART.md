...
This guide brings up the OniLink `v0.2.0-beta.1` public beta in front of one BDS `1.26.44.3` + Endstone `0.11.9` backend. The included exact Linux native profile is production-approved and still fails closed on every mismatched runtime detail; the application packet monitor remains beta.
...
- The [current beta](https://github.com/TheNINJALLO/OniLink/releases/tag/v0.2.0-beta.1).
...
1. Stop BDS.
2. Copy `onibridge-0.2.0-beta.1-bds-1.26.44.3-linux-x86_64.so` into the Endstone `plugins/` directory.
