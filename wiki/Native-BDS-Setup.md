...
> [!WARNING]
> `v0.1.7` is production-approved only for BDS `1.26.44.3` Linux x86-64 with Endstone `0.11.9`; a nearby version is not compatible. The exact profile reports `production_ready=true` and every mismatch still fails closed.
...
install -m 0755 \
  ...
  /srv/bds/plugins/onibridge-0.1.7-bds-1.26.44.3-linux-x86_64.so
```
