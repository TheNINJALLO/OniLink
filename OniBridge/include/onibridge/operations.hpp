...
class CommandCompatibilityMonitor final {
  public:
    ...
    [[nodiscard]] std::uint64_t registry_updates() const noexcept {
        return registry_updates_.load();
    }
    [[nodiscard]] std::uint64_t soft_enum_updates() const noexcept {
        return soft_enum_updates_.load();
    }
    [[nodiscard]] std::int64_t last_registry_update_ms() const noexcept {
        return last_registry_update_ms_.load();
    }
    [[nodiscard]] static constexpr bool alters_command_packets() noexcept {
        return false;
    }
...
class RateLimitedSecurityLogger final {
  public:
    ...
    void
    log(std::string key, std::string_view severity, std::string_view message, std::int64_t now_ms);
...
class LegacyDataMigrator final {
  public:
    [[nodiscard]] static MigrationPlan plan(const std::filesystem::path& source,
