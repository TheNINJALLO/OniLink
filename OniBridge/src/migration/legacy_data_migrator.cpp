...
MigrationPlan LegacyDataMigrator::plan(const std::filesystem::path& source,
...
void LegacyDataMigrator::apply(const MigrationPlan& plan, bool explicit_confirmation) {
    if (!explicit_confirmation || !plan.safe_to_apply)
        throw std::runtime_error("migration is not explicitly confirmed or safe");
