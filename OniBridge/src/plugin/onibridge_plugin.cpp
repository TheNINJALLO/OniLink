...
#define ONIBRIDGE_VERSION "0.1.1"
#endif
...
public:
    void onEnable() override {
        try {
            ...
            if (!config_.previous_key_id.empty()) {
                keys.previous = ForwardingKey{config_.previous_key_id, load_secret(config_.previous_secret)};
            }
            service_ = std::make_unique<OniBridgeService>(
                config_.bridge_id,
            ...
            registerEvent(&OniBridgePlugin::onPacketReceive, *this, endstone::EventPriority::Monitor);
            ...
            if (getServer().getName() != "Endstone" || getServer().getVersion() != "0.11.9") {
                hook_error_ = "this adapter requires exact Endstone 0.11.9; unknown native runtimes fail closed";
            ...
            if (!hook_active_) {
                ...
                if (config_.shutdown_on_hook_failure) getServer().shutdown();
            } else {
                getLogger().info("OniBridge native identity hook is active for the exact reviewed profile.");
            }
    ...
    void onDisable() override {
        if (hook_active_) {
            ...
            if (!uninstall_generated_native_login_hook(error)) {
                getLogger().critical("OniBridge could not safely remove its native hook: {}", error);
            }
        }
        service_.reset();
        hook_active_ = false;
    ...
    bool onCommand(
        ...
        if (command.getName() != "onibridge" || args.empty()) return false;
        const auto& action = args[0];
        if (action == "status") {
            sender.sendMessage("OniBridge hook active: {}", hook_active_ ? "true" : "false");
            if (!hook_error_.empty()) sender.sendErrorMessage("Compatibility: {}", hook_error_);
        } else if (action == "version") {
            sender.sendMessage("OniBridge {}", ONIBRIDGE_VERSION);
        } else if (action == "profile") {
            sender.sendMessage("Required profile: {}", config_.required_profile.empty() ? "none" : config_.required_profile);
            sender.sendMessage("Production validation: {}", hook_active_ ? "passed" : "failed");
        } else if (action == "sessions") {
            sender.sendMessage("Verified active sessions: {}", service_ ? service_->identities().size() : 0);
        } else if (action == "identity") {
            if (args.size() != 2 || !service_) return false;
            const auto identity = service_->identities().by_player_name(args[1]);
            if (!identity) sender.sendErrorMessage("No verified identity for that player.");
            else sender.sendMessage("{}: XUID {}, proxy {}, backend {}", identity->player_name, identity->xuid, identity->proxy_id, identity->backend_name);
            ...
            if (error) sender.sendErrorMessage("{}", *error);
        } else if (action == "command-status") {
            sender.sendMessage("Backend command registry observed: {}", command_monitor_.registry_updates() ? "true" : "false");
            sender.sendMessage("Last command registry update: {}", command_monitor_.last_registry_update_ms());
            sender.sendMessage("Soft enum updates observed: {}", command_monitor_.soft_enum_updates());
    ...
    void onPacketReceive(endstone::PacketReceiveEvent& event) {
        if (!service_ || event.isCancelled() || event.getPacketId() != 1 || event.getSubClientId() != 0) return;
        const auto envelope = LoginEnvelopeParser::parse(event.getPayload(), 1'048'576, config_.maximum_token_size);
        if (!envelope) {
        ...
        const auto decision = service_->stage_forwarded_login(
            envelope.envelope->forwarding_token,
            address.getHostname(),
    ...
    void onPlayerLogin(endstone::PlayerLoginEvent& event) {
        if (!service_ || !config_.verify_post_login_xuid) return;
        ...
        if (!identity || identity->xuid != event.getPlayer().getXuid()) {
            if (identity) service_->identities().remove_session(identity->session_id);
    ...
    void onPlayerQuit(endstone::PlayerQuitEvent& event) {
        if (!service_) return;
        if (const auto identity = service_->identities().by_player_name(event.getPlayer().getName())) {
...
ENDSTONE_PLUGIN("onibridge", ONIBRIDGE_VERSION, onistone::onibridge::OniBridgePlugin) {
    ...
    command("onibridge")
        ...
        .usages(
            "/onibridge status", "/onibridge version", "/onibridge profile",
            "/onibridge identity <player: string>", "/onibridge sessions",
            "/onibridge test-config", "/onibridge command-status")
