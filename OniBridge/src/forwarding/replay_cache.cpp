...
ReplayCache::ReplayCache(std::size_t maximum_entries) : maximum_entries_(maximum_entries) {
    if (maximum_entries == 0) throw std::invalid_argument("replay cache maximum must be positive");
}
...
bool ReplayCache::consume(const ForwardingClaims& claims, std::int64_t now_ms) {
    ...
    for (auto it = shard.entries.begin(); it != shard.entries.end();) {
        if (it->second < now_ms) { it = shard.entries.erase(it); size_.fetch_sub(1); }
        else ++it;
    }
    if (shard.entries.contains(identity)) return false;
    auto observed = size_.load();
    do {
        if (observed >= maximum_entries_) return false;
...
std::size_t ReplayCache::size() const noexcept { return size_.load(); }
