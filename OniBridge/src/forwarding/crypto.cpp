...
Digest sha256(std::span<const std::byte> input) {
    ...
    for (int shift = 56; shift >= 0; shift -= 8)
        padded.push_back(static_cast<std::byte>((bit_length >> shift) & 0xff));
...
bool constant_time_equal(std::span<const std::byte> left,
    ...
    for (std::size_t i = 0; i < count; ++i)
        different |= std::to_integer<unsigned>(left[i] ^ right[i]);
    return different == 0;
}
...
std::optional<std::vector<std::byte>> base64url_decode(std::string_view input) {
    if (input.empty() || input.find('=') != std::string_view::npos || input.size() % 4 == 1)
        return std::nullopt;
    ...
    if (bits && (accumulator & ((1u << bits) - 1u)) != 0)
        return std::nullopt;
    return result;
}
