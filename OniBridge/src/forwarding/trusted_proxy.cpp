...
bool parse_ip(std::string_view text, std::array<std::uint8_t, 16>& output, bool& ipv4) {
    ...
    if (inet_pton(AF_INET6, value.c_str(), v6.data()) == 1) {
        ...
        if (std::equal(mapped.begin(), mapped.end(), v6.begin())) {
            output.fill(0);
            std::copy(v6.begin() + 12, v6.end(), output.begin());
            ipv4 = true;
        } else {
            output = v6;
            ipv4 = false;
    ...
    if (inet_pton(AF_INET, value.c_str(), v4.data()) == 1) {
        output.fill(0);
        std::copy(v4.begin(), v4.end(), output.begin());
        ipv4 = true;
        return true;
    }
    return false;
...
TrustedProxyMatcher::TrustedProxyMatcher(const std::vector<std::string>& cidrs) {
...
void TrustedProxyMatcher::add(std::string_view cidr) {
    ...
    if (slash == std::string_view::npos) {
        throw std::invalid_argument("trusted proxy entry must be CIDR");
    }
    Network network;
    if (!parse_ip(cidr.substr(0, slash), network.address, network.ipv4)) {
        throw std::invalid_argument("invalid trusted proxy address");
    }
    unsigned prefix = 0;
    ...
    if (error != std::errc{} || end != prefix_text.data() + prefix_text.size() ||
        prefix > maximum) {
        throw std::invalid_argument("invalid trusted proxy prefix");
    }
...
bool TrustedProxyMatcher::matches(std::string_view address) const {
    ...
    bool ipv4 = false;
    if (!parse_ip(address, candidate, ipv4)) {
        return false;
    }
    for (const auto& network : networks_) {
        ...
        if (!std::equal(candidate.begin(), candidate.begin() + bytes, network.address.begin())) {
            continue;
        }
        const auto mask = static_cast<std::uint8_t>(0xff << (8 - bits));
        if (bits == 0 || (candidate[bytes] & mask) == (network.address[bytes] & mask)) {
            return true;
        }
    }
    return false;
