...
LoginEnvelopeResult fail(std::string error) {
  ...
  public:
    ...
    bool read_u32_le(std::uint32_t& result) {
        if (remaining() < 4)
            return false;
        result =
            static_cast<std::uint8_t>(input_[offset_]) |
...
class JsonCursor final {
  public:
    ...
    bool top_level_strings(std::string& name, std::string& token, std::string& error) {
        spaces();
        if (!take('{'))
            return set(error, "JWT payload is not a JSON object");
        spaces();
        if (take('}'))
            return set(error, "JWT payload has no identity fields");
        bool saw_name = false;
        ...
        while (true) {
            ...
            spaces();
            if (!take(':'))
                return set(error, "JSON member is missing a colon");
            spaces();
            if (key == "ThirdPartyName" || key == "OniForward") {
                std::string value;
                if (!string(value, error))
                    return set(error, "required login field is not a JSON string");
                auto& seen = key == "ThirdPartyName" ? saw_name : saw_token;
                if (seen)
                    return set(error, "duplicate required login field");
                seen = true;
            ...
            if (take('}'))
                break;
            if (!take(','))
                return set(error, "JSON object separator is invalid");
            spaces();
        }
        spaces();
        if (offset_ != input_.size())
            return set(error, "JWT payload has trailing JSON data");
  ...
  private:
    ...
    bool string(std::string& output, std::string& error) {
        if (!take('"'))
            return set(error, "expected a JSON string");
        output.clear();
        while (offset_ < input_.size()) {
            ...
            if (ch == '"')
                return true;
            if (ch < 0x20)
                return set(error, "JSON string contains a control byte");
            if (ch != '\\') {
                output.push_back(static_cast<char>(ch));
                continue;
            }
            if (offset_ >= input_.size())
                return set(error, "truncated JSON escape");
            ...
            case 'u': {
                if (input_.size() - offset_ < 4)
                    return set(error, "truncated JSON unicode escape");
                ...
                for (int index = 0; index < 4; ++index) {
                    ...
                    if (digit >= '0' && digit <= '9')
                        ...
                        return set(error, "invalid JSON unicode escape");
                }
                if (codepoint >= 0xd800 && codepoint <= 0xdfff)
                    return set(error, "JSON surrogate escapes are not accepted");
                if (codepoint <= 0x7f)
            ...
            default:
                return set(error, "invalid JSON escape");
            }
        }
    ...
    bool literal(std::string_view wanted) {
        if (input_.substr(offset_, wanted.size()) != wanted)
            return false;
        offset_ += wanted.size();
        return true;
    }
    ...
    bool value(unsigned depth, std::string& error) {
        if (depth > 32 || offset_ >= input_.size())
            return set(error, "JSON nesting or value is invalid");
        if (input_[offset_] == '"') {
            std::string ignored;
        ...
        if (input_[offset_] == '{') {
            ...
            while (true) {
                ...
                spaces();
                if (!take(':'))
                    return set(error, "nested JSON object is invalid");
                spaces();
                if (!value(depth + 1, error))
                    return false;
                spaces();
                if (take('}'))
                    return true;
                if (!take(','))
                    return set(error, "nested JSON object separator is invalid");
                spaces();
            }
        }
        if (input_[offset_] == '[') {
            ...
            while (true) {
                ...
                if (take(']'))
                    return true;
                if (!take(','))
                    return set(error, "JSON array separator is invalid");
                spaces();
            }
        }
        if (literal("true") || literal("false") || literal("null") || number())
            return true;
...
LoginEnvelopeResult LoginEnvelopeParser::parse(std::string_view packet_payload,
    ...
    if (!packet.skip(4))
        return fail("Login packet is missing the protocol version");
    ...
    if (!packet.read_u32_le(auth_size) || auth_size > packet.remaining() ||
        !packet.skip(auth_size)) {
    ...
    std::string error;
    if (!JsonCursor(json).top_level_strings(
            envelope.player_name, envelope.forwarding_token, error)) {
