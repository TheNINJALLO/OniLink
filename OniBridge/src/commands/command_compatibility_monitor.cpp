...
void CommandCompatibilityMonitor::observed_soft_enum_update() noexcept {
    soft_enum_updates_.fetch_add(1);
}
...
void RateLimitedSecurityLogger::log(std::string key,
                                    std::string_view severity,
                                    std::string_view message,
                                    std::int64_t now_ms) {
    std::scoped_lock lock(mutex_);
    const auto found = last_log_.find(key);
    if (found != last_log_.end() && now_ms - found->second < interval_ms_)
        return;
