...
from .profile import ProfileError, create_candidate, read_json, validate_profile, write_json
...
def parser() -> argparse.ArgumentParser:
    ...
    candidate.add_argument("--platform", choices=("linux-x86_64", "windows-x86_64"), required=True)
    ...
    auth.add_argument("--platform", choices=("linux-x86_64", "windows-x86_64"), required=True)
    auth.add_argument("--validation-rva", type=lambda value: int(value, 0), required=True)
    auth.add_argument("--validation-size", type=lambda value: int(value, 0), required=True)
    ...
    auth.add_argument("--authentication-size", type=lambda value: int(value, 0), required=True)
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        if args.command == "inspect":
            ...
            value = {
                "format": image.file_format, "architecture": image.architecture, "abi": image.abi,
            ...
            value = create_candidate(args.binary, args.version, args.platform, args.signature, args.minimum_patch_length, args.role, evidence)
            write_json(args.output, value)
            print(json.dumps({"output": str(args.output), "status": value["validation_status"], "blockers": value["release_blockers"]}, indent=2))
            if value["validation_status"] != "production":
                return 3
        elif args.command == "validate-profile":
            validate_profile(read_json(args.profile), args.binary, production=not args.allow_candidate)
            print(json.dumps({"status": "valid", "profile": str(args.profile)}, indent=2))
        elif args.command == "generate-headers":
            generate_minimal_header(read_json(args.abi), args.output)
            print(json.dumps({"status": "generated", "header": str(args.output)}, indent=2))
            ...
            if set(xrefs) != {"auth", "uuid", "xid", "realms"}:
                raise ProfileError("auth analysis requires auth, uuid, xid, and realms xrefs")
            paths = generate_auth_artifacts(
                args.binary, args.version, args.platform, args.validation_rva, args.validation_size,
                args.call_rva, args.helper_rva, args.helper_size, args.authentication_size,
                args.string_size, args.signature, xrefs, args.output)
            print(json.dumps({"status": "candidate", "outputs": {key: str(value) for key, value in paths.items()}}, indent=2))
            return 3
        elif args.command == "generate-adapter":
            generate_adapter(read_json(args.profile), read_json(args.abi), args.output)
            print(json.dumps({"status": "generated", "adapter": str(args.output)}, indent=2))
        return 0
    except (OSError, ValueError, BinaryError, ProfileError, json.JSONDecodeError) as exc:
