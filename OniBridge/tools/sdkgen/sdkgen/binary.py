...
class Image:
    ...
    def executable_section_for_rva(self, rva: int, length: int = 1) -> Section | None:
        return next(
            (s for s in self.sections if s.executable and s.contains_rva(rva, length)),
            None,
        )
...
def _load_elf(path: Path, data: bytes) -> Image:
    ...
    for index in range(shnum):
        ...
        name_offset, section_type, flags, address, file_offset, size = (
        ...
        file_backed_size = (
            0 if section_type == 8 else size
        )  # SHT_NOBITS has no bytes in the executable.
        if file_backed_size and file_offset + file_backed_size > len(data):
            raise BinaryError(f"ELF section {name!r} exceeds the file")
        sections.append(
            Section(name, address, file_offset, file_backed_size, bool(flags & 0x4))
        )
...
def _load_pe(path: Path, data: bytes) -> Image:
    ...
    for index in range(section_count):
        ...
        sections.append(
            Section(name, rva, raw_offset, size, bool(characteristics & 0x20000000))
        )
    if not any(section.executable and section.size for section in sections):
        raise BinaryError("PE has no executable section")
    return Image(
        path, "PE32+", "x86_64", "microsoft-x64", image_base, data, tuple(sections)
    )
