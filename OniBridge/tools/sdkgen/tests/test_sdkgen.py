...
from sdkgen.profile import PRODUCTION_GATES, ProfileError, create_candidate, validate_profile


SIGNATURE = bytes.fromhex("55 48 89 e5 48 83 ec 20")


def elf_fixture(code: bytes = SIGNATURE, machine: int = 62, executable: bool = True) -> bytes:
    ...
    struct.pack_into("<IIQQQQIIQQ", data, 0x240, 1, 1, 0x6 if executable else 0x2, 0x1000, 0x100, 0x40, 0, 0, 16, 0)
    struct.pack_into("<IIQQQQIIQQ", data, 0x280, 7, 3, 0, 0, 0x180, len(names), 0, 0, 1, 0)
    return bytes(data)


def pe_fixture(code: bytes = SIGNATURE, machine: int = 0x8664, executable: bool = True) -> bytes:
    data = bytearray(0x300)
    data[:2] = b"MZ"
    ...
    struct.pack_into("<IIIIIIHHI", data, section + 8, 0x100, 0x1000, 0x100, 0x200, 0, 0, 0, 0, 0x60000020 if executable else 0x40000040)
...
class ProfileTests(unittest.TestCase):
    ...
    def candidate(self, signature: str = "55 48 89 E5 48 83 EC 20", patch_length: int = 8):
        return create_candidate(self.binary, "fixture", "linux-x86_64", signature, patch_length, "test", self.evidence)
    ...
    def test_invalid_instruction_boundary_is_release_blocker(self):
        ...
        profile = create_candidate(self.binary, "fixture", "linux-x86_64", "55 48 89 E5", 8, "test", evidence)
    ...
    def test_unsupported_abi(self):
        with self.assertRaisesRegex(ProfileError, "does not match"):
            create_candidate(self.binary, "fixture", "windows-x86_64", "55 48", 5, "test", self.evidence)
